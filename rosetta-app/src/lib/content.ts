...
export const FIVETRAN_PILLARS: Pillar[] = [
  ...
  },
  {
    ...
    flag: "Rationalizing Fivetran Transformations and dbt Cloud scheduling is an active post-merger decision — present the sync-aware trigger as fact, not the unified roadmap.",
  },
  {
    key: "deployment",
    ...
    flag: "HVR is no longer a separate product line — its log-based high-volume replication capability was folded into the standard connector/CDC story.",
  },
  {
    key: "security",
    ...
    body: "Private connectivity that never touches the public internet (PrivateLink on AWS / Azure / GCP), SSH and reverse-SSH tunnels for firewalled sources, VPN, or direct connection with IP allow-listing. Column-level controls let you hash PII irreversibly or block columns so they never leave the source. Certifications include SOC 2 Type II, ISO 27001, HIPAA, GDPR.",
  ...
  },
  {
    ...
    body: "Setup flow: pick source, authenticate, choose destination, set network mode, select schemas/tables/columns, set frequency, run the initial historical sync, then ongoing incremental syncs. Pricing is MAR — Monthly Active Rows — distinct rows inserted/updated/deleted in a month, counted once per table. High-churn tables drive cost; selecting fewer objects reduces it.",
    nodes: ["Historical sync", "Incremental sync", "MAR (Monthly Active Rows)", "Schema/table/column selection", "Dashboard / sync health"],
...
export const DBT_PILLARS: Pillar[] = [
  {
    ...
    owner: "dbt",
    why: "Same shape as self-hosted scripts vs a managed platform — it decides how fast and how governed a customer's transformation layer is.",
  ...
  },
  {
    ...
    nodes: ["Source (raw landed table)", "Staging model", "ref() → DAG", "Materialization type", "Snapshot (SCD2)", "Package"],
  },
  {
    key: "testing",
...
export const TERMS: TermPair[] = [
  ...
  { ft: "Sync", dbt: "Run / Build ( dbt build )", note: "Both mean 'the job executes,' but a sync moves rows IN; a run transforms rows already there. Inbound vs in-place." },
  { ft: "Destination", dbt: "Target / Profile / Adapter", note: "Same warehouse, two sides. Fivetran writes TO the destination; dbt's target is HOW it connects to and runs on that same warehouse." },
...
export const MYTHS: Myth[] = [
  ...
  { audience: "dbt people", myth: "\"Pricing is per-seat everywhere.\"", truth: "Fivetran prices on MAR — distinct rows changed per month. Benchmarking 'per user' misses the row-volume world entirely." },
];
